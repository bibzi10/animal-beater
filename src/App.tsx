...
const [sparks, setSparks] = useState<Array<{id:number,x:number,y:number,angle:number,color:string}>>([]);
...
// Werte für die Farm
const autoFarmPrice = 250;
const farmUpgradePrice = Math.floor(200 * Math.pow(1.6, farmLevel - 1));
const farmIncome = farmLevel * 10;
...
// Effekt für die automatische Farm (alle 3 Sekunden)
useEffect(() => {
let interval: ReturnType | undefined;
if (hasAutoFarm) {
interval = setInterval(() => {
setCount(c => c + farmIncome);
setCoins(c => c + farmIncome);
}, 3000);
}
return () => { if (interval) clearInterval(interval); };
}, [hasAutoFarm, farmIncome]);

const simulatePet = (x: number, y: number, amount: number) => {
setCount(c => c + amount);
setCoins(c => c + amount);
if (petRef.current) {
setIsJumping(false);
...
{isLightMode ? '🌙 Dark' : '☀️ Light'}
  ...
  {sparks.map(s => {
    const xVar = `${Math.cos(s.angle * Math.PI / 180) * 60}px`;
    const yVar = `${Math.sin(s.angle * Math.PI / 180) * 60}px`;
    ...
    return <div key={s.id} className="spark" style={style} />;
  })}
    ...
    <section className="glass-tile side-panel">
      ...
      <div className="shop-section" style={{ marginTop: '20px' }}>
        ...
        <div className={`shop-content ${isShopOpen ? 'open' : ''}`}>
          ...
          {!hasAutoPetter ? (
            ...
            <div className="upgrades">
              <button
                ...
                style={{ width: '100%', marginTop: '10px', ['--active-color' as any]: currentAnimal.color } as any}
              >
          ...
          {/* NEU: AUTO-FARM SEKTION */}
          {!hasAutoFarm ? (
            <button
              className="shop-btn"
              onClick={() => { if (coins >= autoFarmPrice) { setCoins(c => c - autoFarmPrice); setHasAutoFarm(true); } }}
              disabled={coins < autoFarmPrice}
              style={{ width: '100%', padding: '10px', marginTop: '20px', borderRadius: '8px', border: 'none', background: coins >= autoFarmPrice ? '#60a5fa' : '#333', color: '#000', fontWeight: 'bold' }}
            >
              Buy Auto-Farm ({autoFarmPrice} 🪙)
            </button>
          ) : (
            <button
              className="shop-btn"
              onClick={() => { if (coins >= farmUpgradePrice) { setCoins(c => c - farmUpgradePrice); setFarmLevel(l => l + 1); } }}
              disabled={coins < farmUpgradePrice}
              style={{ width: '100%', padding: '10px', marginTop: '10px', borderRadius: '8px', border: 'none', background: coins >= farmUpgradePrice ? '#60a5fa' : '#333', color: '#000', fontWeight: 'bold' }}
            >
              Upgrade Farm ({farmUpgradePrice} 🪙)
              <div style={{ fontSize: '0.7rem' }}>Yield: {farmIncome} / 3s</div>
            </button>
          )}
        </div>
      </div>
